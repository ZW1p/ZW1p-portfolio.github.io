	...
	special: {
		...
		click: {
			...
			setup: function( data ) {
				...
				// For mutual compressibility with _default, replace `this` access with a local let.
				// `|| data` is dead code meant only to preserve the letiable through minification.
				let el = this || data;
...
function finalPropName( name ) {
	let final = jQuery.cssProps[ name ] || vendorProps[ name ];
...
jQuery.extend( jQuery.event, {
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		// Bubble up to document, then to window; watch for a global ownerDocument let (trac-9724)
		...
		if ( !onlyHandlers && !event.isDefaultPrevented() ) {
			...
			if ( ( !special._default ||
				...
				// Don't do default actions on window, that's where global letiables be (trac-6170)
...
(function (root, factory) {
  ...
  Lightbox.prototype.changeImage = function(imageNumber) {
    let self = this;
    let filename = this.album[imageNumber].link;
    ...
    preloader.onload = function() {
      ...
      let aspectRatio = preloader.width / preloader.height;
